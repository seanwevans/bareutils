...
section .bss
    utmp_buf    resb UTMP_SIZE            ; Buffer to hold one utmp record
    username    resb UT_NAMESIZE          ; Buffer to hold username
...
_start:
    ...
    xor rdx, rdx                          ; No mode needed for O_RDONLY
    syscall

    test rax, rax
    js exit_error                         ; Jump if negative (error)
...
read_loop:
    ...
    mov r12, 0                            ; r12 will track if we need a space before the username
...
first_username:
    ...
    mov rdx, UT_NAMESIZE
    sub rdx, rcx
    dec rdx                               ; rdx now contains username length

...
section .bss
    inbuf:        resb 3            ; Input buffer for 3 bytes
    outbuf:       resb 8192         ; Output buffer
    filepath:     resb 256          ; Buffer for filepath
...
_start:
    ...
    cmp rax, 0                      ; Check if there are any args left
    jle use_stdin                   ; If no args, read from stdin

    pop rsi                         ; Get argv[1] (filename)

    mov rax, SYS_OPEN
    mov rdi, rsi                    ; filename
    mov rsi, O_RDONLY               ; read-only
    xor rdx, rdx                    ; no special mode needed
    syscall
...
process_input:
    mov rax, SYS_READ
    mov rdi, r14                    ; Input descriptor (file or stdin)
    lea rsi, [inbuf]

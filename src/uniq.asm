; uniq.asm - Implementation of the uniq command in NASM x86_64 assembly
; Removes duplicate adjacent lines from input
; Usage: ./uniq [input_file [output_file]]
; If input_file is not specified, reads from stdin
; If output_file is not specified, writes to stdout

%include "include/sysdefs.inc"

; Additional constants that we need
...
_start:
    ; Initialize file descriptors to default values
...
process_next_line:
    ...
    ; Check if we have a previous line to compare with
    ...
    ; Lines have same length, compare content
    mov rsi, current_line
    mov rdi, previous_line
    mov rcx, [current_line_len]
    ...
    ; Lines are identical, skip writing and continue
    jmp process_next_line
    
first_line:
    ; This is the first line, so write it and save it
...
lines_differ:
    ; Lines are different, write current line
    mov rdi, [output_fd]
    mov rsi, current_line
    mov rdx, [current_line_len]
    write rdi, rsi, rdx
    
    ; Make current line the new previous line
    call save_current_line
    
    jmp process_next_line

; Helper function to save current line as previous line
save_current_line:
    mov rsi, current_line
    mov rdi, previous_line
    mov rcx, [current_line_len]
...
end_processing:
    ; Close files if not standard streams
    mov rdi, [input_fd]
...
; Returns a line in current_line and its length in current_line_len
; Sets current_line_len to 0 on EOF
read_line:
...
buffer_has_data:
    ...
    ; Check if we've exceeded the line buffer size
...
read_line_end:
    ; Add newline to the current line
    mov rdi, current_line
    add rdi, [current_line_len]
